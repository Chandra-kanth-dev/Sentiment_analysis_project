...
VOCAB_SIZE  = 10000
MAX_LENGTH  = 50
MODEL_PATH  = "/tmp/mental_health_rnn_model.h5"
DATA_PATH   = "/tmp/mental_health_dataset.csv"
GDRIVE_ID   = "1whOvUTQlR3oQ5bL46awDL5HQs1QfTLrA"
DATASET_SLUG = "suchintikasarkar/sentiment-analysis-for-mental-health"
...
CLASS_META = {
    ...
    "Depression":           {"color": "#3B82F6", "icon": "😔", "tip": "You are not alone — please reach out to a trusted person or mental health professional."},
    ...
    "Stress":               {"color": "#F97316", "icon": "⚡", "tip": "Prioritize rest, set boundaries, and consider stress-management techniques."},
    "Suicidal":             {"color": "#EF4444", "icon": "🆘", "tip": "Please reach out NOW — iCall: 9152987821 | Vandrevala: 1860-2662-345"},
}
...
.hero {
    ...
    padding: 3rem 2rem 2.5rem; text-align: center; margin-bottom: 2.2rem;
}
.hero::before {
...
.hero-badge {
    ...
    padding: 0.3rem 1rem; font-size: 0.72rem; letter-spacing: 2px;
    text-transform: uppercase; color: #818cf8; margin-bottom: 1rem;
}
.hero h1 {
    font-family: 'Syne', sans-serif; font-size: clamp(1.8rem, 5vw, 2.6rem);
    font-weight: 800; color: #fff; line-height: 1.15;
    margin-bottom: 0.6rem; letter-spacing: -0.5px;
}
.hero h1 span { color: #818cf8; }
.hero p { color: #94a3b8; font-size: 0.95rem; max-width: 480px; margin: 0 auto; font-weight: 300; }

.section-label {
    font-family: 'Syne', sans-serif; font-size: 0.72rem;
    letter-spacing: 2.5px; text-transform: uppercase; color: #64748b; margin-bottom: 0.6rem;
}

.stTextArea > label { display: none; }
.stTextArea textarea {
    background: #0d1424 !important; color: #f1f5f9 !important;
    border: 1px solid #1e2d45 !important; border-radius: 12px !important;
    font-family: 'DM Sans', sans-serif !important; font-size: 1rem !important;
    line-height: 1.6 !important; resize: vertical !important; transition: border-color .2s !important;
}
.stTextArea textarea:focus { border-color: #6366f1 !important; box-shadow: 0 0 0 3px rgba(99,102,241,.12) !important; }
...
.stButton > button {
    ...
    padding: 0.75rem 2rem !important; transition: opacity .2s, transform .15s !important;
}
.stButton > button:hover { opacity: 0.88 !important; transform: translateY(-1px) !important; }
.stButton > button:active { transform: translateY(0) !important; }

.result-card {
    background: #0d1424; border: 1px solid #1e2d45; border-radius: 16px;
    padding: 1.8rem; margin: 1.5rem 0 0.5rem; position: relative; overflow: hidden;
}
.result-card::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0;
    height: 3px; background: var(--accent); border-radius: 16px 16px 0 0;
}
.result-eyebrow { font-size: 0.7rem; letter-spacing: 2px; text-transform: uppercase; color: #475569; margin-bottom: 0.4rem; }
.result-class { font-family: 'Syne', sans-serif; font-size: 2.2rem; font-weight: 800; color: var(--accent); line-height: 1.1; margin-bottom: 0.3rem; }
.result-conf { font-size: 0.88rem; color: #64748b; margin-bottom: 1.2rem; }
...
.prob-track { background: #111827; border-radius: 999px; height: 6px; overflow: hidden; }
.prob-fill  { height: 6px; border-radius: 999px; background: var(--bar-color); }
...
.disclaimer {
    background: #080d17; border: 1px solid #1e2d45; border-radius: 10px;
    padding: 0.9rem 1.1rem; color: #475569; font-size: 0.78rem;
...
def load_tokenizer_and_encoder():
    if not os.path.exists(DATA_PATH):
        try:
            ...
            st.error("Kaggle credentials missing. Add [kaggle] username and key to Streamlit secrets.")
            st.stop()

        zip_path = "/tmp/mental_health.zip"
        api_url  = f"https://www.kaggle.com/api/v1/datasets/download/{DATASET_SLUG}"
        r = requests.get(api_url, auth=(kaggle_user, kaggle_key), stream=True, timeout=120)
        if r.status_code != 200:
            st.error(f"Kaggle download failed: {r.status_code} {r.text[:200]}")
            st.stop()
        ...
        with zipfile.ZipFile(zip_path, "r") as z:
            ...
            with z.open(csv_name) as src, open(DATA_PATH, "wb") as dst:
                dst.write(src.read())
...
<div class="hero">
  ...
  <p>Paste any statement below — our SimpleRNN model classifies its mental health sentiment across 7 categories.</p>
</div>
""", unsafe_allow_html=True)
...
# LOAD RESOURCES
# ─────────────────────────────────────────────
with st.spinner("Loading model & preparing tokenizer…"):
    try:
